...
class FalsificationEngine:
    ...
    async def run(
        ...
        *,
        ...
        report.duration_ms = int((time.monotonic() - started) * 1000)
        report.coverage = self._coverage(report, resolved_targets, selected)
        report.settle()
    ...
    @staticmethod
    def _coverage(
        report: FalsificationReport, target_names: list[str], selected: list[StrategyName]
    ) -> AttackSurfaceCoverage:
        """What was actually attacked, per target.

        Every registered target appears, including the six with no strategy that can
        attack them. They report 0%, which is the honest number: a visible gap can be
        planned against, an omitted one cannot.
        """
        executed = {
            StrategyName(name)
            for name in report.strategy_coverage.executed
            if name in {s.value for s in StrategyName}
        }
        entries: list[TargetCoverage] = []

        for name in target_registry.known_targets():
            target = target_registry.get(name)
            if target is None:  # pragma: no cover - registry is the source of names
                continue
            if name not in target_names:
                entries.append(TargetCoverage(target=name, unattacked_by=[]))
                continue

            capable = sorted(s.value for s in target.strategies if s in set(selected))
            attacked = sorted(s.value for s in target.strategies if s in executed)
            counterexamples = sum(
                1 for example in report.counterexamples if example.target == name
            )
            entries.append(
                TargetCoverage(
                    target=name,
                    attacked_by=attacked,
                    unattacked_by=[s for s in capable if s not in attacked],
                    attempts=sum(
                        r.attempts for r in report.strategies if name in (r.targets or [])
                    ),
