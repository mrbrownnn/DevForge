...
Falsification metrics appear in an `eval` report only when a case actually ran a
falsify step. An empty section is noise, and a row of "unknown" for a feature nobody
used reads as a failure rather than an absence.

| Metric | Direction |
| --- | --- |
| Mutation score (weighted by mutant count, not a mean of means) | up |
| Counterexamples found | neutral |
| Falsification failure rate | down |
| Falsification incomplete rate | down |
| Post-repair falsification survival | up |

Failure rate and incomplete rate are tracked separately on purpose: a search that
could not finish is a gap in the evidence, not evidence of a defect, and averaging
